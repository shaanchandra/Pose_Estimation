...
"""
import logging, os, time, datetime, pprint
import threading
import argparse
...
def train(config_yaml,displayiters,saveiters,maxiters,max_to_keep=5):
    ...
    for it in range(max_iter+1):
        ...
        if it % display_iters == 0:
            ...
            cum_loss = 0.0
            logging.info("iteration: {}/{},    loss:  {:.4f},    lr: {},  |   Elapsed Time:  {:0>2}:{:0>2}:{:05.2f},    Time:  {}"
                         .format(it, max_iter, average_loss, current_lr, int(hours),int(minutes),seconds, datetime.datetime.now().strftime("%H:%M")))
